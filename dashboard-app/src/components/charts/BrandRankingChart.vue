    ...
    tooltip: { 
      ...
      formatter: (params) => {
        ...
        return `
          ...
          <div style="font-size: 13px; color: #64748b; margin-bottom: 2px;">所属集团: <span style="color: #334155;">${p.parent}</span></div>
...
const handleClick = (params) => emit('openModal', 'Brand', params.name, marketScope.value)
  ...
  if (data) {
    ...
    data.forEach(r => {
      ...
      if (isStandalone || isSubBrand) {
        if (!temp[r.brand]) temp[r.brand] = { parent: r.parent_group, cy: 0, ly: 0, shareCy: 0, shareLy: 0, hasNext: false }
        if (r.report_year === props.selectedYear + 1) { 
          temp[r.brand].ly += (r.monthly_units_ly || 0); temp[r.brand].shareLy += (r.monthly_share_ly || 0); temp[r.brand].hasNext = true 
        } else if (r.report_year === props.selectedYear) { 
          temp[r.brand].cy += (r.monthly_units_cy || 0); temp[r.brand].shareCy += (r.monthly_share_cy || 0) 
        }
      }
    })
    
    rankingData.value = Object.keys(temp).map(n => ({
      name: n, parent: temp[n].parent,
